...
func parse257(resp *Response) (dirname string, err error) {
	...
	// PJS added Wed May 14 10:08:59 MDT 2014
	// Ubuntu: 14.10 - vsftpd sends back a full path like /home/bob/whatever
	if resp.Message[0:2] == "\"/" {
		if len(resp.Message) > 2 {
			return resp.Message[1:len(resp.Message)-1], nil
		} else {
			return "", nil
		}
	}
